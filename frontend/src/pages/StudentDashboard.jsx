...
const StudentDashboard = () => {
    ...
    const fetchLaundryData = async () => {
        if (!user?.token) return;
        try {
            const res = await fetch(`${API}/laundry/my-bookings`, {
                headers: { 'Authorization': `Bearer ${user.token}` }
            });
            const data = await res.json();
            if (data.success) setLaundryBookings(data.data);
        } catch (err) {
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-indigo-100/60 text-base font-medium mb-6 max-w-2xl leading-relaxed">
                            {application
                                ...
                                : "Welcome to the DormDesk Hostel Management System. Start by submitting your application."}
                        </p>
...
const DashboardView = ({ user, student, application, laundryBookings, facilityBookings, myAllocation, notices, navigate, onProfilePicDelete, uploadingProfile, myStatus }) => {
    return (
        <div className="space-y-12 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            {/* Welcome Banner */}
            <div className="relative overflow-hidden bg-indigo-900 rounded-[3rem] p-10 text-white shadow-2xl shadow-indigo-900/20">
                <div className="absolute top-0 right-0 w-96 h-96 bg-white/5 rounded-full -mr-32 -mt-32 blur-3xl animate-pulse"></div>
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                    <div className="space-y-4 text-center md:text-left">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-[10px] font-black uppercase tracking-widest border border-white/10">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
                            Live Updates — DormDesk Portal
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                {/* Right - Security Pulse */}
                <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-[3rem] p-10 border border-white/40 shadow-2xl shadow-indigo-500/5">
                    <div className="flex items-center justify-between mb-8">
                        <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Security Pulse</h3>
                        <div className={`flex items-center gap-2 px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm ${myStatus?.status === 'INSIDE' ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
                    ...
                    <div className="space-y-10">
                        <div className="flex items-center gap-6 group">
                            <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center text-2xl transition-transform group-hover:scale-110 ${myStatus?.status === 'INSIDE' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                                <HiOutlineArrowsRightLeft />
                            </div>
                            <div className="space-y-1">
                                <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Last Activity</div>
                                <div className="text-lg font-black text-slate-800 dark:text-white tracking-tight">{myStatus?.lastAction || 'No recent scans'}</div>
                                <div className="text-[11px] font-bold text-slate-400">{myStatus?.lastTime ? new Date(myStatus.lastTime).toLocaleString() : '—'}</div>
                            </div>
                        </div>

                        {myStatus?.status === 'OUTSIDE' && myStatus?.destination && (
                            <div className="p-8 bg-slate-50 dark:bg-slate-800/50 rounded-[2rem] border border-slate-100 dark:border-white/5 space-y-4">
                                ...
                                <div className="pl-6 border-l-2 border-rose-200">
                                    <p className="text-base font-black text-slate-700 dark:text-white leading-tight">
                                        {myStatus.destination}
                                    </p>
                                    {myStatus.goingHome && (
                                        <div className="mt-2 inline-flex items-center gap-2 px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full text-[9px] font-black uppercase tracking-widest">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-[3rem] p-10 border border-white/40 shadow-2xl shadow-indigo-500/5">
                    ...
                    <div className="grid grid-cols-2 gap-8">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {(student?.submittedMonths?.filter(m => m.status === 'Accepted') || []).length > 0 ? (
                                    student.submittedMonths
                                        ...
                                        .map((mon, idx) => (
                                            <span key={idx} className="px-4 py-2 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-2xl text-[11px] font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-800">
                                                {mon}
                                            </span>
                                        ))
                                ) : (
                                    <div className="w-full h-12 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-dashed border-slate-200 dark:border-slate-700 flex items-center justify-center text-[10px] font-bold text-slate-400 uppercase italic">
                        ...
                        <div className="space-y-4">
                            <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Hostel Deposit</div>
                            <div className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-white/5">
                                <div className="text-3xl font-black text-slate-800 dark:text-white tracking-tighter">
                                    LKR {student?.refundPayment?.amount?.toLocaleString() || '0'}
                                </div>
                                <div className="text-[9px] font-black text-indigo-500 uppercase tracking-widest mt-2">Fully Refundable</div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-[3rem] p-10 border border-white/40 shadow-2xl shadow-indigo-500/5 flex flex-col justify-center items-center text-center space-y-6">
                    <div className="w-20 h-20 rounded-[2rem] bg-amber-50 text-amber-600 flex items-center justify-center text-4xl shadow-inner animate-bounce">
                        <HiOutlineMegaphone />
                    </div>
                    <div className="space-y-2">
...
const StatCard = ({ icon: Icon, label, value, color, onClick }) => {
    ...
    return (
        ...
        >
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-2xl mb-5 shadow-inner group-hover:scale-110 transition-transform ${colors[color]}`}>
                <Icon />
            </div>
            <div className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">{label}</div>
            <div className="text-2xl font-black text-slate-800 dark:text-slate-100 mt-1 tracking-tight">{value}</div>
        </div>
    );
};
...
const Input = ({ label, ...props }) => (
    <div className="space-y-2">
        <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] px-1">{label}</label>
        <input {...props} className="w-full px-5 py-4 bg-slate-50/50 dark:bg-slate-800/50 border border-slate-100/50 dark:border-white/5 rounded-2xl focus:ring-4 focus:ring-indigo-100 dark:focus:ring-indigo-900/20 focus:border-indigo-400 dark:focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-800 outline-none transition-all font-bold text-slate-700 dark:text-slate-200 shadow-sm" />
    </div>
);
...
const Select = ({ label, options, ...props }) => (
    <div className="space-y-2">
        <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] px-1">{label}</label>
        <div className="relative">
            <select {...props} className="w-full px-5 py-4 bg-slate-50/50 dark:bg-slate-800/50 border border-slate-100/50 dark:border-white/5 rounded-2xl focus:ring-4 focus:ring-indigo-100 dark:focus:ring-indigo-900/20 focus:border-indigo-400 dark:focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-800 outline-none transition-all font-bold text-slate-700 dark:text-slate-200 appearance-none shadow-sm">
...
const TextArea = ({ label, ...props }) => (
    <div className="space-y-2">
        <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] px-1">{label}</label>
        <textarea {...props} rows={3} className="w-full px-5 py-4 bg-slate-50/50 dark:bg-slate-800/50 border border-slate-100/50 dark:border-white/5 rounded-2xl focus:ring-4 focus:ring-indigo-100 dark:focus:ring-indigo-900/20 focus:border-indigo-400 dark:focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-800 outline-none transition-all font-bold text-slate-700 dark:text-slate-200 shadow-sm" />
    </div>
);
...
const ReadField = ({ label, value, span }) => (
    <div className={`${span || ''} flex flex-col gap-2`}>
        <span className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] px-1">{label}</span>
        <div className="bg-slate-50/50 dark:bg-slate-800/30 border border-slate-100 dark:border-white/5 rounded-2xl px-4 py-3 min-h-[48px] flex items-center shadow-inner group hover:border-indigo-100 dark:hover:border-indigo-900/50 transition-colors">
            {value ? (
                <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{value}</span>
            ) : (
                <span className="text-xs font-medium text-slate-300 dark:text-slate-600 italic">Not provided</span>
            )}
        </div>
    </div>
);
                    ...
                    {filteredComplaints.length > 0 ? filteredComplaints.map(c => {
                        ...
                        return (
                            <div
                                ...
                                className={`p-5 rounded-[2rem] border transition-all cursor-pointer flex items-center gap-4 group ${selectedId === c._id ? 'bg-indigo-600 border-indigo-500 text-white shadow-2xl shadow-indigo-500/20' : 'bg-white/50 dark:bg-slate-900/50 border-slate-100/50 dark:border-white/5 hover:border-indigo-100 dark:hover:border-indigo-500/30 hover:shadow-xl hover:-translate-y-0.5'}`}
                            >
                                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-lg font-black flex-shrink-0 ${selectedId === c._id ? 'bg-white/20 text-white' : 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 shadow-inner'}`}>
                                    {c.title.charAt(0).toUpperCase()}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start mb-1">
                                        <h4 className={`text-sm font-black truncate ${selectedId === c._id ? 'text-white' : 'text-slate-800 dark:text-slate-100'}`}>
                                            {c.title}
                                        </h4>
                                    </div>
                                    <p className={`text-[11px] font-bold truncate mb-1 opacity-60 ${selectedId === c._id ? 'text-white' : 'text-slate-400 dark:text-slate-500'}`}>
                                        {lastMsg?.content || 'No messages yet'}
                                    </p>
                                    <div className={`text-[9px] font-black uppercase tracking-[0.2em] ${selectedId === c._id ? 'text-white/40' : 'text-slate-300 dark:text-slate-600'}`}>
                                        {new Date(c.updatedAt).toLocaleDateString()}
                                    </div>
                                </div>
                                <div className="flex flex-col items-end gap-2">
                                    {c.studentUnreadCount > 0 && (
                                        <div className="min-w-[24px] h-6 px-2 rounded-full bg-rose-500 text-white text-[10px] font-black flex items-center justify-center shadow-lg shadow-rose-500/40 animate-pulse">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button
                                            ...
                                            className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg shadow-sm transition-all active:scale-95 ${selectedId === c._id ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-rose-50 dark:bg-rose-900/30 text-rose-500 dark:text-rose-400 hover:bg-rose-100 opacity-0 group-hover:opacity-100'}`}
                                            title="Delete"
                                        >
                                            <HiOutlineTrash />
                                        </button>
                ...
                {selectedId && activeComplaint ? (
                    <>
                        ...
                        {activeComplaint.status !== 'resolved' ? (
                            <form onSubmit={sendMessage} className="p-8 border-t border-slate-100 dark:border-white/5 bg-white/50 dark:bg-slate-900/50 flex gap-4 items-end">
                                <div className="flex-1 relative">
                                    {selectedFile && (
                                        <div className="absolute bottom-full mb-4 left-0 right-0 animate-in slide-in-from-bottom-4 duration-500">
                                            <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-white/10 rounded-3xl p-4 shadow-[0_20px_50px_rgba(0,0,0,0.1)] flex items-center justify-between">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-12 h-12 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-2xl flex items-center justify-center overflow-hidden">
                                                    ...
                                                    <div>
                                                        <div className="text-xs font-black text-slate-800 dark:text-slate-200 truncate max-w-[200px]">{selectedFile.name}</div>
                                                ...
                                                <button
                                                    ...
                                                    className="p-3 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-2xl text-slate-400 hover:text-rose-500 transition-all active:scale-90"
                                                >
                                    ...
                                    <button
                                        ...
                                        className="absolute left-4 bottom-4 p-2.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-xl transition-all active:scale-90"
                                    >
                                    ...
                                    <textarea
                                        ...
                                        rows={1}
                                        className="w-full pl-16 pr-5 py-5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-white/5 rounded-3xl text-sm font-bold focus:ring-4 focus:ring-indigo-100 dark:focus:ring-indigo-900/20 focus:border-indigo-400 outline-none resize-none transition-all dark:text-slate-200 shadow-inner"
                                        style={{ maxHeight: 120, minHeight: '64px' }}
                                    />
                                </div>
                                <button
                                    type="submit"
                                    disabled={(!message.trim() && !selectedFile) || sending}
                                    className="w-16 h-16 bg-indigo-600 text-white rounded-[1.5rem] flex items-center justify-center hover:bg-indigo-700 hover:shadow-2xl hover:shadow-indigo-500/30 disabled:opacity-30 transition-all active:scale-90 shadow-xl"
                                >
